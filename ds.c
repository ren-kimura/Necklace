...
//---Node_t (tree)---
Node_t* create_node(bool is_path, u64 id, Node_t* parent) {
    Node_t* new_node = (Node_t*)malloc(sizeof(Node_t));
    if (new_node == NULL) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        exit(EXIT_FAILURE);
    }
    new_node->is_path = is_path;
    new_node->id = id;
...
int add_child(Node_t* parent, Node_t* child) {
    ...
    fprintf(stderr, "Error: cannot add a child %ld to an already full parent %ld\n", child->id, parent->id);
    return 0; // failure
}
...
void free_tree(Node_t* node) {
    ...
    free(node); // free the root of this subtree
}