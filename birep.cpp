...
INT rc(INT hash, INT K)
{
    /* constant time calculation of reverse complement */
    /* ref-> https://www.biostars.org/p/113640/#424278 */
    INT res = ~hash; // take NOT
...
public:
    ...
    {
        logfilename = remove_extension(filename, ".fa") + ".uebi" + to_string(K) + ".log";
...
public:
      ...
      kmers(&pool), heads(&pool)
    {
        ...
        logfilename = remove_extension(filename, ".fa") + ".obi" + to_string(K) + "." + to_string(option) + ".log";
        kmers.reserve(reserve_size);
        cout << "\nUsing pool size: " << pool_size / (1024 * 1024) << " MB\n";
        cout << "K-mers reserve size: " << reserve_size << "\n";
    };
    ...
    REP process() {
        ...
        auto total_time = get_kmers_time.count(); // interim
    ...
    void get_kmers(VINT& kmerv) {
        ...
        for (INT i = 0; i < (INT)(reads.size()); ++i) {
            ...
            if (j == INF) continue; // if not any valid k-mer, go to next read
            hash = encode_kmer(read.substr(j, K));
            INT rc_hash = rc(hash, K); // hash of reverse complement
            auto it1 = kmers.find(hash);
            auto it2 = kmers.find(rc_hash);
            if (it1 == kmers.end() && it2 == kmers.end()) {
                INT canon_hash = min(hash, rc_hash);
                kmers[canon_hash] = id;
                kmerv.emplace_back(canon_hash);
                ++id;
                if (id == INF) {
                    cerr << "Too much k-mers!!\n\n";
                    exit(1);
        ...
        /* debug : display bi-directed de Bruijn graph (START) */
        cout << "Displaying de Bruijn graph:\n";
        INT N = kmers.size();
        for (INT i = 0; i < N; ++i) {
            INT hash = kmerv[i], rc_hash = rc(kmerv[i], K);
            cout << i << "(" << decode_kmer(hash, K) << "," << decode_kmer(rc_hash, K) << "): ";
            for (const auto& c: base) {
                INTB next = step(kmerv, i, c, true, true);
                if (auto j = next.first; j != INF)
                    cout << j << "(" << decode_kmer(kmerv[j], K) << ","
                         << decode_kmer(kmerv[rc(kmerv[j], K)], K) << ") ";
                next = step(kmerv, i, c, false, true);
                if (auto j = next.first; j != INF)
                    cout << j << "(" << decode_kmer(kmerv[j], K) << ","
                         << decode_kmer(kmerv[rc(kmerv[j], K)], K) << ") ";
            }
        ...
        /* debug : display bi-directed de Bruijn graph (END) */

        finished("Detecting k-mers");
        cout << "\n";
    }

    INTB step(const VINT& kmerv, INT id, char c, bool from_canonical, bool is_forward) {
        INT hash = INF;
        if (from_canonical) {
            hash = kmerv[id];
        } else {
        ...
        if (hash == INF) return {INF, true};

        if (c != 'A' && c != 'C' && c != 'G' && c != 'T') return {INF, true};
        ...
        if (auto it = kmers.find(hash); it != kmers.end() && it->second != id)
            return {it->second, true}; // to canonical
        else if (auto it = kmers.find(rc(hash, K)); it != kmers.end() && it->second != id)
            return {it->second, false}; // to non-canonical
        else
            return {INF, true}; // no branch
    }
/*
    bool bfs (const VINT& kmerv, const VINT& match_u, const VINT& match_v, VINT& dist) {
    ...
    bool dfs (const VINT& kmerv, VINT& match_u, VINT& match_v, 
            VINT& dist, INT u) {
        for (auto c: base) {
            auto v = step(kmerv, u, c, true);
            if (v == INF) continue;

            if (match_v[v] == INF || (dist[match_v[v]] == dist[u] + 1 && dfs(kmerv, match_u, match_v, dist, match_v[v]))) {
                match_u[u] = v;
                match_v[v] = u;
                return true;
            }
    ...
    INT hopcroft_karp(const VINT& kmerv, VINT& match_u, VINT& match_v) {
        INT N = (INT)kmers.size();
        VINT dist(N); // distance of bfs
        INT M = 0;
        match_u.assign(N, INF);
        match_v.assign(N, INF);
        while (bfs(kmerv, match_u, match_v, dist)) {
            for (INT u = 0; u < N; ++u) {
                if (match_u[u] == INF && dfs(kmerv, match_u, match_v, dist, u))
                    M++; // found an augpath
            }
    ...
    REP unsorted(const VINT& kmerv, const VVINT& cycles, const VVINT& paths) {
        INT N = (INT)kmers.size();
        ...
        for (INT i = 0; i < P; ++i){
            ...
            for (auto c: base) {
                ...
                if (step(kmerv, head, c, false) != INF)
                    is_root_path = false;
            }
        ...
        for (const auto& cycle: cycles) {
            for (const auto& node: cycle) {
                for (const auto& c: base) {
                    auto next = step(kmerv, node, c, true);
    ...
    VINT find_new_cycle(const VINT& kmerv, const VVINT& paths, Vint& in_pord,  Vint& is_leaf, Vint& visited, INT start) {
        struct Frame {
            INT current;
            INT idx;
            INT b_idx;
        ...
        while (!stack.empty()) {
            ...
            if (top.b_idx < 4) {
                ...
                INT next_node = step(kmerv, cur_node, c, true);
    ...
    REP sorted(const VINT& kmerv, VVINT& cycles, VVINT& paths) {
        INT N = (INT)kmers.size();
        INT C = (INT)cycles.size();
        INT P = (INT)paths.size();
        INT S = N + C + P, ncycs = 0;
        ...
        while (INT res = count(in_ord.begin(), in_ord.end(), 0)) {
            ...
            for (INT i = 0; i < P; ++i) {
                ...
                Vint vis(P, 0);
                VINT pids = find_new_cycle(kmerv, paths, in_ord, is_leaf, vis, i);
                if (!pids.empty()) {
    ...
    string write_subtree(INT root_pid,
        ...
        while (!st.empty()) {
            ...
            for (; fr.base_idx < 4; ++fr.base_idx) {
                ...
                INT nxt = step(kmerv, node, c, true);
                if (nxt == INF) continue;
    ...
    REP forest(const VINT& kmerv, VVINT& cycles, VVINT& paths) {
        string txt;
        INT C = (INT)cycles.size();
        INT P = (INT)paths.size();
        ...
        for (const auto& pid: roots) {
            txt += decode_kmer(kmerv[paths[pid][0]], K).substr(0, K - 1)
                   + write_subtree(pid, paths, kmerv, heads, added)
                   + ",";
        ...
        for (const auto& cycle: cycles) {
            ...
            for (const auto& node: cycle) {
                ...
                for (const auto& c: base) {
                    ...
                    txt += "(" + write_subtree(npid, paths, kmerv, heads, added) + ")";
                }
            }
        ...
        while (INT res = count(added.begin(), added.end(), 0)) {
            ...
            for (INT i = 0; i < P; ++i) {
                ...
                Vint vis(P, 0);
                VINT pids = find_new_cycle(kmerv, paths, added, is_leaf, vis, i);
                if (!pids.empty()) {
...
*/
    ...
    void write(const REP& rep) {
        ...
        string txtfilename = remove_extension(filename, ".fa") + ".obi" + to_string(K) + "." + to_string(option) + ".txt";
        ofstream txtfile(txtfilename);
        if (!txtfile) {
            cerr << "Error: Could not open file " << txtfilename << " for writing.\n";
            return;
        ...
        if (rep.second.empty()) {
            ...
            pntfilename = remove_extension(filename, ".fa") + ".obi" + to_string(K) + "." + to_string(option) + "p.txt";
